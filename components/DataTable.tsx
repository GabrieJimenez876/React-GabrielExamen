...
import { FaSearch, FaFilter, FaChevronLeft, FaChevronRight, FaChevronUp, FaChevronDown } from "react-icons/fa";
...
export function DataTable<T extends Record<string, any>>({
	...
	const filtrados = useMemo(() => {
		...
		return datos.filter((fila) => Object.values(fila).some((v) => String(v).toLowerCase().includes(k)));
	}, [datos, busqueda]);
	...
	const celdaEncabezado = (col: Columna<T>) => (
		<th
			key={col.key}
			className={`px-4 py-3 text-sm font-semibold text-white ${col.alineacion === "right" ? "text-right" : col.alineacion === "center" ? "text-center" : "text-left"}`}
			style={{ width: col.ancho }}
		>
			<button
				onClick={() => alternarOrden(col)}
				className={`inline-flex items-center gap-1 ${col.ordenable ? "cursor-pointer" : "cursor-default"}`}
				aria-label={`Ordenar por ${col.encabezado}`}
			>
				<span>{col.encabezado}</span>
				{col.ordenable &&
					(claveOrden === col.key ? dirOrden === "asc" ? <FaChevronUp size={16} /> : <FaChevronDown size={16} /> : <FaChevronUp size={16} className="opacity-30" />)}
			</button>
		</th>
	);
				...
				<table className="min-w-full">
					<thead className={`${encabezadoFijo ? "sticky top-0" : ""} bg-sky-700`}>
						<tr>
							...
							{acciones?.length ? <th className="px-4 py-3 text-sm font-semibold text-white text-right">Acción</th> : null}
						</tr>
					</thead>
								...
								{columnas.map((col) => (
									<td
										...
										className={`px-4 py-3 text-sm text-slate-700 ${
											col.alineacion === "right" ? "text-right" : col.alineacion === "center" ? "text-center" : "text-left"
										}`}
									>
						...
						.map((_, i) => {
							...
							return (
								<button
									key={n}
									onClick={() => setPagina(n)}
									className={`w-9 h-9 rounded-xl border text-sm ${pagina === n ? "bg-sky-600 text-white border-sky-600" : "border-slate-200 hover:bg-slate-100"}`}
								>
									{n}
								</button>
							);
